  ...
  return {
    async propertySearchAgent({ query, userId }) {
      const [{ parsePropertyQuery }, searchModule, sessionModule] = await Promise.all([
        import("../Week 02 — Natural Language Property Search/propertySearch"),
        import("../Week 03 — MLS Database Integration/searchListings"),
        import("../Week 04 — Conversational Property Search Agent/session"),
      ]);
    ...
    async marketStatsAgent({ query, userId }) {
      ...
      const { getSession } = await import(
        "../Week 04 — Conversational Property Search Agent/session"
      );
    ...
    async recommendationAgent({ userId }) {
      const [{ getSession }, recommendationModule] = await Promise.all([
        import("../Week 04 — Conversational Property Search Agent/session"),
        import("../Week 07 — Recommendation Engine/recommendation"),
      ]);
